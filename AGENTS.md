...
- Firebase を含む各種サービスについて、公式 CLI・API・既存スクリプトで安全に実行できる操作は、手順案内だけで止めず Codex が積極的に実行し、可能な範囲で検証と後始末まで代行する。秘密情報の本人入力、CAPTCHA、権限付与、取り消し困難な外部変更など、ユーザー本人の操作または実行直前の確認が必要な境界だけを依頼する
